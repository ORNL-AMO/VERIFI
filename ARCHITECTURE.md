...
Production v1 status is a v1-owned read model under [`src/app/v1/status`](src/app/v1/status). A pure evaluator accepts an immutable account-workspace snapshot, the current account's canonical calendarization result, an explicit evaluation date, and the current workspace revision. It emits stable finding codes and structured evidence; presentation copy, Todo inclusion, and v1 destinations are resolved separately by the presentation catalog. [`WorkspaceCalendarizationService`](src/app/v1/shared/calendarization/workspace-calendarization.service.ts) owns one revision-safe, account-wide base calculation in stable units and site-energy basis. Status consumes that base directly; v1 workbenches select a meter or facility slice and synchronously project it into the requested display units, site/source basis, and fiscal calendar. Display-only setting changes reproject the base without creating another Worker or retained cache entry. [`WorkspaceStatusService`](src/app/v1/status/workspace-status.service.ts) publishes only evaluations that match the active account and workspace revision. Both services expose explicit `idle`, `evaluating`, `ready`, and `error` states. v0 continues to use the legacy status and calendarization services. The product contract is [V1 Data Validation and Status Rules](docs/v1-data-validation-rules.md).
...
VERIFI uses Bootstrap, ng-bootstrap, Font Awesome, Angular templates, and Plotly. Legacy v0 feature components live under `src/app/v0/` and own their local templates and styles. Version-neutral helpers and contracts live under `src/app/shared/` and should be imported with `@shared/*`; v0-only reusable UI lives under `src/app/v0/shared/` and should be imported with `@v0/shared/*`. Current v0 shared UI includes legacy helper pipes, spinners, table/dropdown helpers, labels, meter content, data-quality displays, analysis presentation widgets, report widgets, settings/help UI, and similar presentation bundles. Do not import from `@v0/*` in root shared, data, domain, platform, or production v1 code. Global style layers live under [`src/styles`](src/styles) and cover tables, forms, navigation, reports, printing, Plotly, and other cross-feature patterns.
...
Weather access is a version-neutral remote-data boundary under [`src/app/platform/weather`](src/app/platform/weather). It owns location search, nearby-station lookup, station response validation, cancellable hourly-data retrieval, and response mapping without retaining UI selection state. Deterministic monthly weather calculations live under [`src/app/domain/calculations/weather`](src/app/domain/calculations/weather). Both v0 and v1 use these boundaries: v0 retains its public adapter behavior while the v1 Predictor workspace owns generation, reviewed reconciliation, and atomic persistence orchestration. V1 derives each facility's weather-station workbenches from existing Weather predictors with the same non-empty `weatherStationId`; no station-group record is persisted. Membership, source, reading, and analysis-reference changes for one derived group commit through one three-store transaction and publish one workspace revision. Station Readings is likewise a derived month-by-output matrix: its range controls review and atomically reconcile calculated records for every included output, while adding a month calculates every output and explicit per-output overrides remain manual records. New-station Setup retains the initial range because a workbench has no Readings tab before creation. When both new-station range fields are empty, station selection previews 24 inclusive calendar months ending in the previous complete month and writes that fallback range into Setup when the station is confirmed. Weather ranges may extend into the future; those months are stored as refreshable calculated zeroes without source-gap warnings, and weather-source requests are capped at the current month. Adding, editing, or deleting one displayed month reconciles all affected `predictorData` records in one native transaction and publishes only the committed reload.

The current production UI is treated as v0 and is lazy-loaded through `src/app/v0/`. The root `AppComponent` owns startup, lifecycle, root routing, and app-wide runtime overlays only; v0 owns the legacy header, legacy route outlet, and legacy UI modals. On unified-ux work, v1 production UI is lazy-loaded through `src/app/v1/` instead of adding v0/v1 conditionals to legacy components.

The unified UI/UX migration keeps the current experience as v0 and adds the new opt-in experience under `/v1`. v0 and v1 should have separate UI components and route shells. P1 prototype code under `/p1` is reference material only and may temporarily use v0 shared UI; production v1 code must not import from `@v0/*`. Shared data services, models, calculations, backups, imports, exports, Web Workers, and Electron boundaries remain outside versioned UI folders unless a dedicated compatibility issue changes those contracts. See the [Unified UI/UX migration guide](docs/unified-ux-migration.md) for the lightweight migration decision process.
