...
export class BetterClimateExcelWriterService {
  ...
  */
  ...
  exportToExcel(report: IdbAccountReport, account: IdbAccount, betterClimateReport: BetterClimateReport) {
    ...
    let requestURL: string = 'Emissions-Reporting';
  ...
  writePortfolioEmissions(workbook: ExcelJS.Workbook, account: IdbAccount, report: IdbAccountReport, betterClimateReport: BetterClimateReport) {
    ...
    worksheet.getCell('D5').value = account.sustainabilityQuestions.greenhouseReductionPercent / 100;

    worksheet.getCell('D6').value = report.baselineYear;

    worksheet.getCell('D7').value = account.sustainabilityQuestions.greenhouseReductionTargetYear;

    //calander/fiscal year
    if (account.fiscalYear == "calendarYear") {
      worksheet.getCell('J5').value = 'Calendar Year (December 31)';
    } else {
      worksheet.getCell('J5').value = this.getFiscalYearEnd(account.fiscalYearMonth);
    }
    //market or location goal?
    if (report.betterClimateReportSetup.emissionsDisplay == 'location') {
      worksheet.getCell('J6').value = 'Location-Based GHG Goal';
    } else {
      worksheet.getCell('J6').value = 'Market-Based GHG Goal';
    }

    if(account.sustainabilityQuestions.energyReductionGoal){
      console.log()
      worksheet.getCell('J7').value = account.sustainabilityQuestions.energyReductionPercent / 100;
    }
  ...
  writeYearColumn(worksheet: ExcelJS.Worksheet, columnLetter: string, yearDetail: BetterClimateYearDetails) {
    ...
    //sq ft
    worksheet.getCell(columnLetter + '12').value = yearDetail.totalSquareFeet;
    //number of facilities
    worksheet.getCell(columnLetter + '13').value = yearDetail.facilityIds.length;

    //Section 2: Absolute Emissions
    //Scope 1 Emissions
    worksheet.getCell(columnLetter + '17').value = yearDetail.emissionsResults.totalScope1Emissions;
    //stationary emissions
    worksheet.getCell(columnLetter + '18').value = yearDetail.emissionsResults.stationaryEmissions;
    //mobile emissions
    worksheet.getCell(columnLetter + '19').value = yearDetail.emissionsResults.mobileTotalEmissions;
    //fugitive emissions
    worksheet.getCell(columnLetter + '20').value = yearDetail.emissionsResults.fugitiveEmissions;
    //process emissions
    worksheet.getCell(columnLetter + '21').value = yearDetail.emissionsResults.processEmissions;

    //scope 2 emissions (location-based)
    worksheet.getCell(columnLetter + '22').value = yearDetail.emissionsResults.locationElectricityEmissions;

    //scope 2 emissions (market-based)
    worksheet.getCell(columnLetter + '23').value = yearDetail.emissionsResults.marketElectricityEmissions;
    ...
    worksheet.getCell(columnLetter + '27').value = yearDetail.emissionsResults.totalBiogenicEmissions;
    ...
    worksheet.getCell(columnLetter + '37').value = yearDetail.onSiteGeneratedElectricity;
    //purchased or acquired electricity
    worksheet.getCell(columnLetter + '38').value = yearDetail.purchasedElectricity;
    //physical ppas
    worksheet.getCell(columnLetter + '39').value = yearDetail.pppaElectricity;
    //financial/virtual ppas
    worksheet.getCell(columnLetter + '40').value = yearDetail.vppaElectricity;
    //unbundled recs
    worksheet.getCell(columnLetter + '41').value = yearDetail.RECs;
    ...
    worksheet.getCell(columnLetter + '43').value = fuelVals.usage;
    //district hot water 
    fuelVals = this.getFuelValue(fuelVals.usedFuels, ['District Hot Water'], yearDetail.fuelTotals);
    worksheet.getCell(columnLetter + '44').value = fuelVals.usage;
    ...
    if (otherFuelVals.otherFuels.length == 1 || otherFuelVals.otherFuels.length > 2) {
      ...
      worksheet.getCell('B61').value = cellName;
      worksheet.getCell(columnLetter + '61').value = otherFuelVals.usage;
      ...
      worksheet.getCell('B61').value = 'Other Fuel (' + otherFuelVals.otherFuels[0] + ') (MMBtu)';
      ...
      worksheet.getCell('B62').value = 'Other Fuel (' + otherFuelVals.otherFuels[1] + ') (MMBtu)';
    }
    ...
    worksheet.getCell(columnLetter + '67').value = vehicleFuelVals.usage;
    //Gasoline
    ...
    worksheet.getCell(columnLetter + '68').value = vehicleFuelVals.usage;
    //Diesel
    ...
    if (otherVehicleFuelVals.otherFuels.length == 1 || otherVehicleFuelVals.otherFuels.length > 2) {
      ...
      cellName = cellName + ') (MMBtu)';
      worksheet.getCell('B74').value = cellName;
      worksheet.getCell(columnLetter + '74').value = otherVehicleFuelVals.usage;
      ...
      worksheet.getCell(columnLetter + '74').value = fuelVals.usage;
      worksheet.getCell('B74').value = 'Other Fuels (' + otherVehicleFuelVals.otherFuels[0] + ') (MMBtu)';

      fuelVals = this.getFuelValue([], [otherVehicleFuelVals.otherFuels[1]], yearDetail.fuelTotals);
      worksheet.getCell(columnLetter + '75').value = fuelVals.usage;
      worksheet.getCell('B75').value = 'Other Fuels (' + otherVehicleFuelVals.otherFuels[1] + ') (MMBtu)';
    }
  ...
  writeFacilityEmissions(workbook: ExcelJS.Workbook, report: IdbAccountReport, betterClimateReport: BetterClimateReport) {
    ...
    //C6 Reporting Year
    worksheet.getCell('C3').value = report.reportYear;
    //start row 6
    let rowIndex: number = 6;
