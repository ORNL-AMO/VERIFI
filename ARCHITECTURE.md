...
Weather-station discovery is a version-neutral remote-data boundary under [`src/app/platform/weather`](src/app/platform/weather). It owns location search, nearby-station lookup, station response validation, and mapping into the shared `WeatherStation` contract without retaining UI selection state. Both v0 and v1 may use that boundary; hourly weather retrieval and weather-predictor calculations remain in their existing workflow services until they are migrated separately.
