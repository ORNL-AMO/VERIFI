...
export class MonthlyFacilityAnalysis {
  ...
  getMonthlyAnalysisSummary(selectedGroup: AnalysisGroup, analysisItem: IdbAnalysisItem, facility: IdbFacility, calanderizedMeters: Array<CalanderizedMeter>, accountPredictorEntries: Array<IdbPredictorEntry>): MonthlyAnalysisSummary {
    ...
    let baselineYear: number = this.helperService.getFiscalYear(baselineDate, facility);
    ...
    while (baselineDate < endDate) {
      let fiscalYear: number = this.helperService.getFiscalYear(new Date(baselineDate), facility);
      ...
      analysisSummaryData.push({
        ...
        SEnPI: this.helperService.checkValue(SEnPI),
        savings: this.helperService.checkValue(savings),
        percentSavingsComparedToBaseline: this.helperService.checkValue(percentSavingsComparedToBaseline) * 100,
        yearToDateSavings: this.helperService.checkValue(yearToDateSavings),
        yearToDatePercentSavings: this.helperService.checkValue(yearToDatePercentSavings) * 100,
        rollingSavings: this.helperService.checkValue(rollingSavings),
        rolling12MonthImprovement: this.helperService.checkValue(rolling12MonthImprovement) * 100
      });
      summaryDataIndex++;
