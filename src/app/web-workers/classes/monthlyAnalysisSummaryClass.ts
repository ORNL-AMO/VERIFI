    ...
    getMonthlyAnalysisSummaryData(): Array<MonthlyAnalysisSummaryData> {
        return this.monthlyAnalysisSummaryData.map(summaryDataItem => {
            return {
                ...
                adjustedForNormalization: summaryDataItem.adjustedForNormalization,
                adjusted: summaryDataItem.adjusted,
                baselineAdjustmentForNormalization: summaryDataItem.baselineAdjustmentForNormalization,
                baselineAdjustmentForOther: summaryDataItem.baselineAdjustmentForOther,
                baselineAdjustment: summaryDataItem.baselineAdjustment,
                predictorUsage: summaryDataItem.predictorUsage,
                fiscalYear: summaryDataItem.fiscalYear,
                group: summaryDataItem.group,
                SEnPI: this.helperService.checkValue(summaryDataItem.SEnPI),
                savings: this.helperService.checkValue(summaryDataItem.savings),
                percentSavingsComparedToBaseline: this.helperService.checkValue(summaryDataItem.percentSavingsComparedToBaseline) * 100,
                yearToDateSavings: this.helperService.checkValue(summaryDataItem.yearToDateSavings),
                yearToDatePercentSavings: this.helperService.checkValue(summaryDataItem.yearToDatePercentSavings) * 100,
                rollingSavings: this.helperService.checkValue(summaryDataItem.rollingSavings),
                rolling12MonthImprovement: this.helperService.checkValue(summaryDataItem.rolling12MonthImprovement) * 100,
            }
        })
    }
}