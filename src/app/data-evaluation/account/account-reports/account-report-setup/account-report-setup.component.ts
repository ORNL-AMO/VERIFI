...
export class AccountReportSetupComponent {
  ...
  setYearOptions() {
    ...
    let yearOptions: Array<number> = this.calanderizationService.getYearOptions('all');
  ...
  checkReportYear() {
    if (this.reportType == 'analysis' && this.setupForm.controls.reportYear.value != undefined) {
      this.showReportYearWarning = this.calanderizationService.checkReportYearSelection('all', this.setupForm.controls.reportYear.value);
    }
  }
}
