...
Production v1 status is a v1-owned read model under [`src/app/v1/status`](src/app/v1/status). A pure evaluator accepts an immutable account-workspace snapshot, one account-wide calendarization result, an explicit evaluation date, and the current workspace revision. It emits stable finding codes and structured evidence; presentation copy, Todo inclusion, and v1 destinations are resolved separately by the presentation catalog. [`WorkspaceCalendarizationService`](src/app/v1/status/workspace-calendarization.service.ts) and [`WorkspaceStatusService`](src/app/v1/status/workspace-status.service.ts) publish only results that still match the active account and revision, and expose `idle`, `evaluating`, `ready`, and `error` states. v0 continues to use the legacy status-check classes. The product contract is [V1 Data Validation and Status Rules](docs/v1-data-validation-rules.md).
