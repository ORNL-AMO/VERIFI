...
Feature code reads account-scoped state only through workspace signals and changes selections through `AccountWorkspaceService`. All durable account-workspace writes are routed through [`WorkspaceCommandBoundary`](src/app/account-workspace/workspace-command-boundary.service.ts), which coordinates validation, persistence via domain handlers, a single committed workspace reload, and one committed-change event per user operation. Domain handlers in [`src/app/account-workspace/handlers/`](src/app/account-workspace/handlers/) are the only non-infrastructure callers of repository write methods. Components and feature services must not call repository write methods directly; the architecture-enforcement test in [`src/app/indexedDB/architecture-enforcement.spec.ts`](src/app/indexedDB/architecture-enforcement.spec.ts) enforces this invariant.
